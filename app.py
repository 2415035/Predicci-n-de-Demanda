...
dataset_1_file = st.file_uploader("Subir el primer dataset (delimitado por ;)", type=["csv"])
dataset_2_file = st.file_uploader("Subir el segundo dataset (delimitado por ;)", type=["csv"])

# Procesar el primer archivo si se carga
if dataset_1_file is not None:
    try:
        dataset_1 = pd.read_csv(dataset_1_file, sep=";", quotechar='"', on_bad_lines='skip')  # Leer con delimitador ;
        st.write("Primer dataset cargado con éxito:")
        st.write(dataset_1.head())  # Mostrar las primeras 5 filas
    except Exception as e:
        st.error(f"Error al cargar el primer dataset: {e}")

# Procesar el segundo archivo si se carga
if dataset_2_file is not None:
    try:
        dataset_2 = pd.read_csv(dataset_2_file, sep=";", quotechar='"', on_bad_lines='skip')  # Leer con delimitador ;
        st.write("Segundo dataset cargado con éxito:")
        st.write(dataset_2.head())  # Mostrar las primeras 5 filas
    except Exception as e:
        st.error(f"Error al cargar el segundo dataset: {e}")
